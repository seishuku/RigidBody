...
{
	vec3 center={ 256.0f, 256.0f, 0.0f };
	const float maxRadius=256.0f;
...
void PhysicsIntegrate(RigidBody_t *body, const float dt)
{
	...
	const vec3 acceleration=Vec3_Muls(body->force, body->invMass);
	body->velocity=Vec3_Addv(body->velocity, Vec3_Muls(acceleration, dt));
...
// Apply a force affect to a rigid body, positive force will push away, negative will attract.
void PhysicsAffect(RigidBody_t *body, const vec3 center, const float force)
{
	// Calculate direction from center to rigid body
	vec3 direction=Vec3_Subv(body->position, center);
	Vec3_Normalize(&direction);

	// Force=Mass*Acceleration
	body->force=Vec3_Muls(direction, force*body->mass);
}
